<?php
// Vereist: $pageTitle, $locations (array), optioneel $formErrors, $old
$eventImages = [
...
<head>
    ...
    <title><?= htmlspecialchars($pageTitle ?? 'Nieuw Event'); ?></title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/css/bootstrap.min.css" rel="stylesheet"
          integrity="sha384-sRIl4kxILFvY47J16cr9ZwB07vP4J8+LH7qKQnuqkuIAvNWLzeN8tE5YBujZqJLB" crossorigin="anonymous">
    <link rel="stylesheet" href="<?= URLROOT; ?>/public/css/events.css">
    <style>
        .event-preview .card { border: 0; box-shadow: 0 6px 18px rgba(0,0,0,.08); }
        .event-preview .card-img-top { object-fit: cover; height: 200px; }
                            ...
                            <div class="row g-3">
                                <div class="col-md-8">
                                    ...
                                    <input
                                        ...
                                        value="<?= isset($old['Naam']) ? htmlspecialchars($old['Naam']) : '' ?>">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="date" class="form-control" name="Datum" id="Datum" required
                                           value="<?= isset($old['Datum']) ? htmlspecialchars($old['Datum']) : '' ?>">
                                ...
                                <div class="col-md-8">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control" name="Locatie" id="Locatie"
                                               ...
                                               value="<?= isset($old['Locatie']) ? htmlspecialchars($old['Locatie']) : '' ?>"
                                               required>
                                    </div>
                                ...
                                <div class="col-12 d-flex gap-2">
                                    <button class="btn btn-dark" type="submit">Event Toevoegen</button>
...
(function(){
    ...
    function monthName(nlDate) {
        try { return new Intl.DateTimeFormat('nl-NL', { day: '2-digit', month: 'long', year: 'numeric' }).format(nlDate); }
        catch { return nlDate.toISOString().slice(0,10); }
    }
    ...
    // Client-side validatie (laat browser POST doen; GEEN preventDefault)
    if (form) {
        form.addEventListener('submit', (e) => {
            let ok = true;
            if (naam && !naam.value.trim()) { naam.classList.add('is-invalid'); ok = false; } else if (naam) { naam.classList.remove('is-invalid'); }
            if (datum && !datum.value) { datum.classList.add('is-invalid'); ok = false; }
            if (locatieInput && !locatieInput.value.trim()) { locatieInput.classList.add('is-invalid'); ok = false; } else if (locatieInput) { locatieInput.classList.remove('is-invalid'); }
            if (!ok) e.preventDefault(); // alleen bij invalid tegenhouden
        });
    }
})();
</script>
